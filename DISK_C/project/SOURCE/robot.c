...
void Robotstate_Show(int x,int y,struct robot* ro,char* text,int* flash){
	...
	switch (ro->state) {
		case 0:strcpy(text2,"空闲中");color = LIGHTGREEN;break;
		case 1:strcpy(text2,"前往药架");color = YELLOW;break;
		case 2:strcpy(text2,"取药中");color = LIGHTCYAN;break;
		case 3:strcpy(text2,"前往出药口");color = YELLOW;break;
		default:strcpy(text2,"未知");color = LIGHTGRAY;break;
	}
